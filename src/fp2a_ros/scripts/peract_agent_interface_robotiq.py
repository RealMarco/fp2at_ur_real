...
#	    overhead_depth = image_to_float_array(overhead_depth, 2**24 - 1) # 1000.0
	    ...
	    overhead_point_cloud = torch.tensor([overhead_point_cloud], device=self.device)
	    ...
	    low_dim_state = torch.tensor([[low_dim_np]]) # why three layers [[[]]]?
	...
	def apply_gripper_offset(self, gripper_pose, offset=(0.0, 0.0, -0.214)): # -0.107 Tool Center Point to flange center point
